...
The purpose of the code is to draw diameter of coins measured by each student.
...
for i, coinType in enumerate(studentData.coinType):
	if coinType == 'quarter': 		#Draw quarters.
		if (quarterLabel == 0):		#comes with legend only one time
			ax1.errorbar(studentData.coinDiameter[i], dummyArray[i], xerr=studentData.coinDiameterUncertainty[i]
...
plt.savefig("plots/coinDiameter.png",bbox_inches='tight',dpi=400)
