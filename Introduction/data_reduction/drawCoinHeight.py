...
inverse_variance								= 1/np.array(studentData.coinHeightUncertainty)**2
inverse_variance_sum_each_bin 					= np.histogram(studentData.coinHeight, binEdges, weights=inverse_variance)[0]
...
weights_sum_each_bin_uncertainty	= np.sqrt(np.histogram(studentData.coinHeight, binEdges, weights=weights_squared)[0])

ax3.hist(studentData.coinHeight, binEdges, weights=weights, color = 'b', log=True)
# Indicate error bars!
...
ax3.set_yticks([1, 10, 100, 1000, 10000, 100000, 1000000])							
ax3.set_yticklabels(['1','10','10'+'$^2$','10'+'$^3$','10'+'$^4$','10'+'$^5$','10'+'$^6$'])				# apply y tick labels
